...
- add a required `classpathIdentity` `@Input` to `ProcessGmdTask` so a directly registered task's runtime classpath invalidates up-to-date checks; `GmdGradlePlugin` derives it from the four version inputs, and direct registrants must wire it from their own configuration or the build fails validation
